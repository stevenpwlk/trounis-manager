...
/**
 * Filet de sécurité local (P2, en attendant la vraie persistance Supabase de P3) :
 * une seule partie en cours, sauvegardée dans localStorage à chaque changement d'état,
 * pour qu'un refresh accidentel ne perde pas une saison entière.
 */
const SAVE_KEY = "trounis-manager-save-v1";
...
export function saveGame(state: GameState): void {
  ...
  try {
    ...
    // Stockage indisponible (mode privé, quota) — la partie continue simplement sans filet.
  }
}
...
export function loadGame(): GameState | null {
  ...
  try {
    ...
    if (
      ...
      typeof parsed.journee !== "number" ||
      typeof parsed.clubCode !== "string" ||
      !Array.isArray(parsed.otherFixtures) ||
      !Array.isArray(parsed.otherPlayedResults) ||
      !Array.isArray(parsed.depeches)
    ) {
      // Sauvegarde d'un schéma antérieur (ex. avant l'ajout du barrage/Coupe) — on l'ignore
      // plutôt que de risquer un crash sur des champs manquants.
      return null;
    }
